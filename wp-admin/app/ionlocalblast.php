...
function search_channels($query) {
    try {
        ...
        if ($is_zip) {
            ...
            // Text search - include slug and domain search
            $search_term = '%' . $query . '%';
            $sql = "SELECT slug, city_name, channel_name, population, state_name, state_code, country_name, country_code, latitude, longitude, custom_domain
                    ...
                    WHERE slug IS NOT NULL 
                    AND city_name IS NOT NULL 
                    AND (city_name LIKE ? OR state_name LIKE ? OR country_name LIKE ? OR channel_name LIKE ? OR slug LIKE ? OR custom_domain LIKE ?)
...
<body>
    <div class="modal-container" style="width:100%;height:100vh;margin:0;padding:0;border-radius:0;overflow:hidden;display:flex;flex-direction:column;">
        <div class="modal-header">
            ...
            <div class="header-right">
                <button class="cart-btn" id="cartBtn" onclick="if(typeof cartManager !== 'undefined') cartManager.showModal(); else toggleCart();" title="Shopping Cart">
        ...
        <div id="selectedItems" class="selected-items hidden">
            <h3>Selected for Distribution</h3>
        ...
        <div class="cart-summary" id="cartSummary" style="display: none;">
            <div class="cart-header">
                <div class="cart-title">Selected Items</div>
            ...
            <div class="cart-actions">
                <button class="btn-clear-cart" onclick="clearDistributionCart()">Clear Cart</button>
    ...
    <script>
        ...
        async function selectVideo(videoId) {
            try {
                ...
                if (data.success) {
                    ...
                    console.log('✅ Video data loaded successfully:', selectedVideo.title);
                    ...
                    // Refresh video results to show selection state
        ...
        async function searchChannels() {
            ...
            try {
                const response = await fetch('', {
                    ...
                    body: `action=search_channels&query=${encodeURIComponent(query)}`
                });
        ...
        function displayChannelResults() {
            ...
            resultsDiv.innerHTML = channelsToShow.map(channel => {
                ...
                const distanceInfo = channel.distance ? `<p><strong>Distance:</strong> ${channel.distance} miles</p>` : '';
                const populationInfo = channel.population ? `<p><strong>Population:</strong> ${channel.population}</p>` : '';
                
                return `
                    <div class="result-card channel-card ${isSelected ? 'selected' : ''}" 
                         onclick="selectChannel('${channel.slug}', '${channel.city_name}', '${channel.channel_name}')"
                         data-channel-slug="${channel.slug}"
                         onmouseenter="event.stopPropagation(); clearHideTimeout(); showChannelPricing('${channel.slug}', this)"
                         onmouseleave="event.stopPropagation(); hideChannelPricing(this)">
                    <div class="card-content">
                        ...
                        ${populationInfo}
                        ${distanceInfo}
                        <div class="channel-pricing-container">
                            <button class="show-pricing-btn" onclick="event.stopPropagation(); toggleChannelPricing('${channel.slug}', this)">
                                <span class="pricing-icon">💰</span>
                                <span class="pricing-text">Show Pricing</span>
                            </button>
                        </div>
                    </div>
                    <div class="channel-pricing-overlay" style="display: none;"></div>
                </div>
                `;
            }).join('');
        ...
        function displaySelectedChannels() {
            const selectedDiv = document.getElementById('selectedChannels');
            
            let html = '';
            
            // Show selected packages
            if (selectedPackages.length > 0) {
                html += '<h4 style="color: #27ae60; margin-bottom: 10px;">📦 Selected Packages:</h4>';
                html += selectedPackages.map(pkg => `
                    <div class="selected-item">
                        <div>
                            <h4>${pkg.name.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h4>
                            <p>${pkg.channels.length} channels</p>
                        </div>
            ...
            // Show selected OTT channels
            if (selectedOTTChannels.length > 0) {
                html += '<h4 style="color: #e74c3c; margin-bottom: 10px;">📡 Selected OTT Channels:</h4>';
                html += selectedOTTChannels.map(channel => `
                    <div class="selected-item">
                        <div>
                            <h4>${channel.name}</h4>
                            <p>${channel.platform}</p>
                        </div>
                        <button class="remove-btn" onclick="removeOTTChannel('${channel.id}')">Remove</button>
                    </div>
                `).join('');
            }
            ...
            if (selectedChannels.length > 0) {
                html += '<h4 style="color: #27ae60; margin: 20px 0 10px 0;">📺 Individual Channels:</h4>';
                html += selectedChannels.map(channel => `
                <div class="selected-item">
                    <div>
                        <h4>${channel.channel_name}</h4>
                        <p>${channel.city_name} (${channel.slug})</p>
                    </div>
                    <button class="remove-btn" onclick="removeChannel('${channel.slug}')">Remove</button>
                </div>
            `).join('');
            }
        ...
        function removeChannel(slug) {
            ...
            if (selectedChannels.length === 0 && selectedPackages.length === 0 && selectedOTTChannels.length === 0 && !selectedVideo) {
                document.getElementById('selectedItems').classList.add('hidden');
                document.getElementById('distributionSettings').classList.add('hidden');
            }
        }
        ...
        function showDistributionSettings() {
            if (selectedVideo && (selectedChannels.length > 0 || selectedPackages.length > 0 || selectedOTTChannels.length > 0)) {
                document.getElementById('selectedItems').classList.remove('hidden');
                document.getElementById('distributionSettings').classList.remove('hidden');
            }
        }
        ...
        function updateDistributionCartDisplay() {
            ...
            // Hide the old footer cart display - everything now goes to header cart
            ...
            // Calculate total
            let total = 0;
            cartItems.innerHTML = distributionCart.map(item => {
        ...
        // Select a pricing option and add to cart
        function selectPricingOption(slug, duration, price) {
            ...
            // Get channel name from the pricing header
            const pricingHeader = event.target.closest('.pricing-content').querySelector('.pricing-header h4');
            const channelName = pricingHeader ? pricingHeader.textContent : `Channel ${slug}`;
            
            // Get all pricing options for this channel
            const pricingOptions = event.target.closest('.pricing-content');
            const pricingData = {};
            const priceElements = pricingOptions.querySelectorAll('.pricing-option .option-price');
            const durationElements = pricingOptions.querySelectorAll('.pricing-option .option-duration');
            
            durationElements.forEach((elem, index) => {
                const duration = elem.textContent.toLowerCase().replace('-', '_');
                const priceText = priceElements[index].textContent.replace('$', '');
                pricingData[duration === 'semi_annual' ? 'semi_annual' : duration] = parseFloat(priceText);
            });
            
            // Add to cart using new cart manager
            if (typeof cartManager !== 'undefined') {
                cartManager.addItem('channel', {
                    id: slug,
                    slug: slug,
                    name: channelName,
                    interval: duration,
            ...
            console.log(`Added ${duration} pricing for ${channelName}: $${price} to cart`);
        }

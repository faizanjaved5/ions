...
async function fallbackToSimpleUpload(fileToUpload, metadata) {
    ...
    try {
        ...
        const result = await uploadWithProgress(formData, fileToUpload);
        
        if (result.success) {
            showProgressWithDetails('Upload complete!', 100, 0, fileToUpload.size, fileToUpload.size);
            setTimeout(() => {
                hideProgress();
                setUploadButtonState('normal');
                console.log('âœ… Upload successful!');
                showCustomAlert('Success', result.message || 'Upload completed successfully!');
            }, 1000);
        } else {
            hideProgress();
            setUploadButtonState('normal');

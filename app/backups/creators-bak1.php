...
if (!empty($search_term)) {
    ...
    // Define search fields for IONLocalVideos
    $search_fields = ['video_id', 'slug', 'title', 'video_link', 'description', 'transcript', 'tags'];
    
    // Parse search term based on type
    if (preg_match('/^"(.+)"$/', $search_term, $matches)) {
        ...
        foreach ($or_terms as $term) {
            ...
            if (!empty($term)) {
                foreach ($search_fields as $field) {
                    $all_field_conditions[] = "$field LIKE %s";
                    $search_params[] = '%' . $term . '%';
                }
            }
        }
...
if (!empty($type_filter)) {
    if ($type_filter === 'upload') {
        $where_conditions[] = "source = 'Upload'";
...
$count_query = "SELECT COUNT(*) FROM IONLocalVideos $where_clause";
...
switch ($sort_option) {
    case 'oldest':
        $order_clause .= 'date_added ASC';
        break;
    case 'title':
        $order_clause .= 'title ASC';
        break;
    case 'status':
        $order_clause .= 'status ASC, date_added DESC';
        break;
    default: // newest
        $order_clause .= 'date_added DESC';
        break;
}
...
$videos_query = "SELECT * FROM IONLocalVideos $where_clause $order_clause LIMIT $per_page OFFSET $offset";
...
<head>
    ...
    .status-options-table {
        ...
        min-width: 300px; /* Reduce min-width to prevent overflow */
    ...
    .status-options-grid {
        ...
        padding: 6px; /* inner gutter handled by parent padding */
    }
    ...
    .video-badges {
        ...
        flex-wrap: wrap;
        gap: 4px;
        margin: 6px 0 2px 0; /* More top margin when after description */
    }
    ...
    .table-action-buttons .badge-dropdown-content {
        ...
        position: fixed !important; /* Use fixed positioning to escape table constraints */
...
?>
    ...
    <script>
        ...
        function initializePreviewForThumb(thumb) {
            const previewContainer = thumb.querySelector('.preview-iframe-container');
        ...
        function loadPreviewIframe(container) {
            ...
            if (!previewUrl || container.querySelector('iframe')) return;
            
            console.log('🎥 Loading preview iframe on demand:', previewUrl);
    ...
    <script>
        ...
        function openIONVideoUploader() {
            ...
            const iframe = document.createElement('iframe');
            iframe.src = 'app/ionuploader.php?v=' + Date.now();
    ...
    .view-btn svg {
        ...
        async function editVideo(videoId) {
            try {
                ...
                // Fetch video data
                const url = `/app/get-video-data.php?id=${videoId}`;
        ...
        function openIONVideoUploaderEdit(videoData) {
            ...
            iframe.src = 'app/ionuploader.php?' + editParams.toString();
            iframe.style.cssText = `
                width: 1200px;
                max-width: 95vw;
                height: 90vh;

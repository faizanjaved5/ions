...
function closeUploaderAndRefreshParent() {
    ...
    if (window.parent && window.parent !== window) {
        try {
            // Close the modal (video already added to page in background)
            const modal = window.parent.document.getElementById('ionVideoUploaderModal');
            if (modal) {
                modal.remove();
                console.log('✅ Modal removed - video already visible on page');
            }
        } catch (e) {
...
function startR2MultipartUpload(file, metadata) {
    ...
    const uploader = new window.R2MultipartUploader({
        ...
        maxRetries: 3,
        endpoint: './ionuploadermultipart3.php', // CRITICAL: Multipart upload backend handler (v3 - encoding fix)
...
// Global object to track detailed upload progress

...
function handleFileSelect(event) {
    ...
    if (validFiles.length === 1) {
        ...
        if (uploadZone) {
            ...
            // CRITICAL FIX: Re-attach click handler after innerHTML replacement
            uploadZone.addEventListener('click', (e) => {
                console.log('📁 File selected area clicked - opening file selector');
        ...
        // Auto-proceed to bulk Step 2 (like single file does)
        console.log('📁 Multiple files selected, auto-proceeding to bulk Step 2');
        setTimeout(() => {
            proceedToBulkStep2();
        }, 500); // Small delay to let file list preparation complete
    }
}
...
function goBackToStep1() {
    ...
    // Hide back button on step 1 (nothing to go back to)
    const backBtn = document.getElementById('backBtn');
    if (backBtn) {
...
function startR2MultipartUpload(file, metadata) {
    ...
    const uploader = new window.R2MultipartUploader({
        ...
        onSuccess: (result) => {
            ...
            // Notify parent that upload is complete (allows modal to close safely)
            if (window.parent && window.parent !== window) {
                window.parent.postMessage({ type: 'upload_complete' }, '*');
            }
...
async function uploadFilesSequentiallyR2(files, index) {
    ...
    try {
        ...
        await uploadSingleFileR2(file, index);
...
function showBulkUploadComplete() {
    console.log('✅ All bulk uploads complete - showing success');
    ...
    // Hide the upload button instead of re-enabling it
    if (nextBtn) {
        nextBtn.style.display = 'none';
        console.log('✅ Upload button hidden after completion');
    }
    
    // Show success alert and close modal after brief delay
    setTimeout(() => {
    ...
    }, 500); // Reduced delay since progress is already complete
}
...
function initializeMobileTabs() {
    ...
    if (thumbnailSection) {
        // Clone thumbnail section for mobile view
        const mobileThumbnailView = thumbnailSection.cloneNode(true);
        mobileThumbnailView.classList.add('mobile-tab-content', 'mobile-thumbnail-view');
        mobileThumbnailView.classList.remove('thumbnail-section');
        mobileThumbnailView.setAttribute('data-tab-content', 'thumbnail');
        mobileThumbnailView.style.display = 'none';
        
        // Ensure the cloned thumbnail controls work
        const clonedUploadBtn = mobileThumbnailView.querySelector('#uploadThumbnailBtn');
        const clonedUrlBtn = mobileThumbnailView.querySelector('#urlThumbnailBtn');
        const clonedGenerateBtn = mobileThumbnailView.querySelector('#generateThumbnailBtn');
        
        if (clonedUploadBtn) {
            clonedUploadBtn.onclick = () => document.getElementById('thumbnailInput').click();
        }
        if (clonedUrlBtn) {
            clonedUrlBtn.onclick = () => document.getElementById('thumbnailUrlOverlay').style.display = 'flex';
        }
        if (clonedGenerateBtn) {
            clonedGenerateBtn.onclick = captureThumbnailFromVideo;
        }
        
        videoPreviewColumn.appendChild(mobileThumbnailView);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nextBtn) {
        ...
        checkNextButton();
        console.log('✅ Next button handler attached and initial state set');
    }
    
    // CRITICAL: Hide Back button on Step 1 (nothing to go back to)
    const backBtn = document.getElementById('backBtn');
    if (backBtn) {
        backBtn.style.display = 'none';
        console.log('✅ Back button hidden on Step 1');

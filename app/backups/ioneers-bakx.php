...
if (!empty($search_term)) {
    ...
    // Define search fields for IONEERS - EXPANDED TO INCLUDE ALL NAME FIELDS
    $search_fields = ['email', 'fullname', 'wp_user_id', 'user_id', 'profile_name', 'handle'];
    ...
    if (preg_match('/^"(.+)"$/', $search_term, $matches)) {
        ...
        $field_conditions = [];
        $collate = 'utf8mb4_general_ci'; // MySQL 5.7; use 'utf8mb4_0900_ai_ci' on MySQL 8+
        
        // Use PDO quote method for proper escaping
        $safe = trim($db->quote($exact_phrase), "'");
        
        foreach ($search_fields as $field) {
            if (in_array($field, ['user_id', 'wp_user_id'], true)) {
                $field_conditions[] = "CAST($field AS CHAR) COLLATE $collate LIKE %s";
            } else {
                $field_conditions[] = "$field COLLATE $collate LIKE %s";
            }
            $search_params[] = "%{$safe}%";
        }
        ...
        // OR SEARCH: john doe (either word) - PROPERLY IMPLEMENTED WITH ESCAPING AND DEDUPLICATION
        $collate = 'utf8mb4_general_ci'; // MySQL 5.7; use 'utf8mb4_0900_ai_ci' on MySQL 8+
        $search_params = [];
        ...
        if ($term_conditions) {
            // AND = require ALL keywords; switch to ' OR ' if ANY keyword should match
            ...
            <div class="add-user-modal-body">
                <form id="add-user-form">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-field">
                            <label for="dob">Date of Birth</label>
                            <input type="text" id="dob" name="dob" value="${dob}" placeholder="MM/DD/YYYY" pattern="\\d{2}/\\d{2}/\\d{4}" title="Please enter date in MM/DD/YYYY format">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-field">
                            <label for="location">Your Location (Public)</label>
                            <input type="text" id="location" name="location" value="${location || ''}" placeholder="City, Country">
                        </div>
                        <div class="form-field">
                            <label for="user_url">Website URL</label>

...
// ============================================
// MEDIA TYPES CONFIGURATION (Required for Google Drive Picker)
// ============================================
const MEDIA_TYPES = {
    video: {
...
// ============================================
// HELPER FUNCTIONS - Category/Visibility Options
// ============================================
/**
 * Generate category options HTML from ION_CATEGORIES
 ...
 */
function generateCategoryOptions(selectedValue = '', includeEmpty = true) {
    if (typeof ION_CATEGORIES === 'undefined') {
        console.warn('⚠️ ION_CATEGORIES not loaded, using fallback categories');
        return `
            <option value="">Select category...</option>
            <option value="Business">ION Business Network</option>
            <option value="Entertainment">ION Entertainment™ Network</option>
            <option value="Education">ION Education™ Network</option>
        `;
    }
    
    let html = includeEmpty ? '<option value="">Select category...</option>\n' : '';
    
    ION_CATEGORIES.forEach(cat => {
        const selected = cat.value === selectedValue ? 'selected' : '';
...
/**
 * Generate visibility options HTML with emojis and descriptions
 * @param {string} selectedValue - The currently selected value (default: 'public')
 * @returns {string} HTML options string
 */
function generateVisibilityOptions(selectedValue = 'public') {
    return `
        <option value="public" ${selectedValue === 'public' ? 'selected' : ''}>🌐 Public - Anyone can view</option>
        <option value="unlisted" ${selectedValue === 'unlisted' ? 'selected' : ''}>🔗 Unlisted - Only with link</option>
        <option value="private" ${selectedValue === 'private' ? 'selected' : ''}>🔒 Private - Only you and admins</option>
    `;
}
...
// Bulk selection handlers (REMOVED DUPLICATE - already defined at line 608)
...
// REMOVED DUPLICATE formatFileSize() - already defined at line 1252
...
async function processPlatformImport(metadata) {
    ...
    console.log('✅ URL validation passed, extracting video ID...');
...
// REMOVED DUPLICATE updateBulkProgress(item) - different signature, keeping version at line 1586
    ...
    if (dropdownItem) {
        ...
        switch(action) {
            ...
            case 'clear-connections':
                ...
                break;
            case 'debug-status':
                debugGoogleDriveStatus();
                break;
            default:
                // Handle drive selection clicks
...
function selectDrive(email) {
    ...
    if (connection) {
        ...
        // Close dropdown and show picker with this drive
        hideGoogleDriveDropdown();
        
        // Show picker with the selected drive's token
...
function clearAllConnections() {
    if (confirm('Are you sure you want to clear all Google Drive connections?')) {
        ...
        if (connectedDrives) {
            connectedDrives.innerHTML = '<div class="no-drives">No drives connected</div>';
        }
    ...
    try {
        gapi.client.init({
            ...
            try {
                tokenClient = google.accounts.oauth2.initTokenClient({
                    ...
                    callback: (response) => {
                        ...
                        if (response.access_token) {
                            ...
                            getUserInfo(response.access_token).then(userEmail => {
                                console.log('👤 User authenticated:', userEmail);
                                storeConnection(userEmail, response.access_token);
                                updateConnectedDrivesDropdown(); // Update dropdown with new connection
                                showGoogleDriveDropdown();
                                showPicker();
                            }).catch(error => {
                                console.error('❌ Error getting user info:', error);
                                storeConnection('Unknown User', response.access_token);
                                updateConnectedDrivesDropdown(); // Update dropdown with new connection
                                showGoogleDriveDropdown();
                                showPicker();
                            });
                            ...
                            if (response.error) {
                                alert('Google Drive authentication failed: ' + response.error);
                            }
                        }
                    },
                    error_callback: (error) => {
                        ...
                        if (error.type === 'redirect_uri_mismatch') {
                            ...
                            console.error('Redirect URI mismatch - Current URI:', window.location.origin + window.location.pathname);
                        } else if (error.type !== 'popup_closed') {
                            alert('Google Drive authentication error: ' + (error.message || 'Unknown error'));
                        } else {
                ...
                alert('Failed to initialize Google authentication: ' + error.message);
            }
            
        }).catch((error) => {
            console.error('❌ GAPI client initialization failed:', error);
            alert('Failed to initialize Google API client: ' + (error.message || 'Unknown error'));
        });
        
    } catch (error) {
        console.error('❌ Google Auth initialization failed:', error);
        alert('Google Drive integration failed to initialize: ' + error.message);
    }
}
...
function authenticateAndShowPicker() {
    ...
    if (!tokenClient) {
        console.log('Token client not ready, retrying in 1 second...');
        setTimeout(() => {
            authenticateAndShowPicker();
        }, 1000);
        return;
    ...
    try {
        ...
        alert('Failed to start Google authentication: ' + error.message);
    }
}
...
function debugGoogleDriveStatus() {
    console.log('=== Google Drive Connection Status ===');
    ...
    console.log('Google APIs loaded:', typeof google !== 'undefined' && typeof gapi !== 'undefined');
    console.log('=====================================');
}
...
function updateConnectedDrivesDropdown() {
    ...
    const noDrivesMessage = document.querySelector('.no-drives'); // FIXED: Match HTML class name
...
// REMOVED DUPLICATE clearAllConnections() - keeping version at line 2579

// Alias for showPicker (used in some places)
function showGoogleDrivePicker() {
    showPicker();
}

// Simple alert wrapper (replace with custom modal if needed)
function showCustomAlert(title, message, actions = null) {
    if (actions && actions.length > 0) {
        const result = confirm(`${title}\n\n${message}`);
        if (result && actions[0] && actions[0].onClick) {
            actions[0].onClick();
        }
    } else {
        alert(`${title}\n\n${message}`);
...
function showPicker() {
    try {
        console.log('📁 Building Google Picker...');
        
        // NEW METHOD: Use Picker API directly (loaded via script tag, not gapi.client.load)
        ...
        console.log('✅ Picker built, showing dialog...');
        picker.setVisible(true);
    } catch (error) {
        console.error('❌ Error showing Google Picker:', error);
        alert('Failed to show Google Drive picker: ' + error.message);
    }
}
...
function handleGoogleDriveSelection(data) {
    ...
    if (data.action === google.picker.Action.PICKED && data.docs.length > 0) {
        if (data.docs.length > 1) {
            ...
            if (typeof window.checkNextButton === 'function') {
                window.checkNextButton();
            } else {
                console.warn('checkNextButton not yet loaded');
            }
            ...
            setTimeout(() => {
                ...
                if (typeof window.proceedToStep2 === 'function') {
                    ...
                    console.error('proceedToStep2 not yet loaded');
                }
...
// REMOVED DUPLICATE uploadGoogleDriveFile() - already defined at line 3352 with async signature
...
// REMOVED DUPLICATE showBulkSummary(errors) - keeping version at line 2451
...
// Initialize Google Drive connections dropdown on page load
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        console.log('🔄 Initializing Google Drive connections...');
        if (typeof updateConnectedDrivesDropdown === 'function') {
            updateConnectedDrivesDropdown();
        }
    });
} else {
    ...
    console.log('🔄 Initializing Google Drive connections...');
    if (typeof updateConnectedDrivesDropdown === 'function') {
        setTimeout(() => updateConnectedDrivesDropdown(), 100); // Small delay to ensure DOM is ready
    }
}

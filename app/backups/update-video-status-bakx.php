...
// Roles.php might not be needed - comment out if it doesn't exist
...
$user_data = $wpdb->get_row("SELECT user_role FROM IONEERS WHERE email = %s", $user_email);
...
try {
    ...
    // Basic debugging
    error_log("VIDEO STATUS UPDATE: Received video_id=$video_id (type: " . gettype($video_id) . "), status=$new_status");
    error_log("VIDEO STATUS UPDATE: Raw POST data: " . print_r($_POST, true));
    ...
    $video = $wpdb->get_row("SELECT id, title, status, user_email FROM IONLocalVideos WHERE id = %d", $video_id);
    ...
    if (!$video) {
        ...
        $video_check = $wpdb->get_row("SELECT id, title, slug FROM IONLocalVideos WHERE id = %d OR slug LIKE %s", $video_id, '%' . $video_id . '%');
        ...
        echo json_encode([
            'success' => false, 
            'error' => "Video not found in database (ID: $video_id)",
            'debug' => [
    ...
    // Update video status
    $result = $wpdb->query("UPDATE IONLocalVideos SET status = %s WHERE id = %d", $new_status, $video_id);

...
class R2MultipartUploader {
    constructor(options = {}) {
        // CRITICAL FIX 2025-10-17: Reduced part size from 100MB to 50MB to prevent timeouts
        this.partSize = options.partSize || 50 * 1024 * 1024; // 50MB parts - smaller = faster, more reliable
        this.maxConcurrentUploads = options.maxConcurrentUploads || 1; // Upload 1 part at a time for stability
        this.maxRetries = options.maxRetries || 3; // 3 retry attempts
        this.retryDelay = options.retryDelay || 2000; // 2 seconds between retries
        this.uploadTimeout = options.uploadTimeout || 900000; // 15 minutes per part (55 KB/s minimum speed)

...
function updateCaptureButtonVisibility() {
    ...
    // Determine if we should show the capture button
    let shouldShow = false;
    let reason = '';
    
    // Check video source and type
    if (currentUploadType === 'file' && selectedFile && currentSource !== 'googledrive') {
        // Local file upload - can capture
        shouldShow = true;
        reason = 'local file upload';
    } else if (currentUploadType === 'import' && (currentSource === 'youtube' || currentSource === 'vimeo')) {
        // Platform imports (YouTube, Vimeo) - these are embedded iframes, cannot capture directly
        // But we already have the thumbnail from the platform
        shouldShow = false;
        reason = 'embedded video (use platform thumbnail or upload custom)';
    } else if (window.editVideoData) {
        // Edit mode - check if video source allows capture
        const source = window.editVideoData.source || '';
        const videoLink = window.editVideoData.video_link || '';
        
        if (source === 'upload' || videoLink.includes('/videos/')) {
            // Uploaded video file - can capture if user has permission
            shouldShow = window.editVideoData.can_capture !== false;
            reason = shouldShow ? 'edit mode with R2 video' : 'edit mode without capture permission';
        } else if (source === 'youtube' || source === 'vimeo' || videoLink.includes('youtube.com') || videoLink.includes('vimeo.com')) {
            // Embedded platform video - cannot capture
            shouldShow = false;
            reason = 'edit mode with embedded video';
        } else {
            // Unknown source in edit mode - show if has permission
            shouldShow = window.editVideoData.can_capture !== false;
            reason = 'edit mode with unknown source';
        }
    } else if (currentSource === 'googledrive') {
        // Google Drive - will be uploaded to R2, but during selection we don't have the video yet
        shouldShow = false;
        reason = 'google drive (upload to R2 first)';
    } else {
        // Default case
        shouldShow = false;
        reason = 'no valid video source';
    }
...
function validateAndProceedToStep2ForImport() {
    const urlInput = document.getElementById('urlInput');
    if (!urlInput) return;
...
function startFileUpload() {
    ...
    if (window.capturedThumbnailBlob) {
        formData.append('thumbnail', window.capturedThumbnailBlob, 'thumbnail.jpg');
        console.log('✅ THUMBNAIL ADDED TO FORMDATA:', window.capturedThumbnailBlob.size, 'bytes');
    } else {

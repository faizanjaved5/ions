...
<body data-theme="<?= $theme ?>" <?= $isIframe ? 'style="margin: 0; padding: 0;"' : '' ?>>
                    ...
                    <div class="metadata-form-column">
                        ...
                        <div class="form-group">
                            <label for="videoDescription">Description
                                <span class="char-count"><span id="descCount">0</span>/3000 characters</span>
                            </label>
                            <textarea id="videoDescription" class="form-input form-textarea" placeholder="Describe your media" maxlength="3000"><?= htmlspecialchars($editVideoData['description'] ?? '') ?></textarea>
                        </div>
    ...
    <script src="<?= $assetBase ?>ionuploaderpro.js?v=<?= $uploaderProVersion ?>&nc=<?= $nuclearBust ?>"></script>         <!-- Advanced features (MUST LOAD FIRST!) -->
    <script src="<?= $assetBase ?>progress-details.js?v=<?= time() ?>&nc=<?= $nuclearBust ?>"></script>                   <!-- Detailed progress tracking -->
    <script src="<?= $assetBase ?>ionuploader.js?v=<?= $uploaderVersion ?>&nc=<?= $nuclearBust ?>"></script>            <!-- Core functionality -->
    ...
    <script src="<?= $assetBase ?>ionuploaderbackground.js?v=<?= time() ?>&nc=<?= $nuclearBust ?>"></script>               <!-- Background upload manager UI widget -->
        ...
        window.addEventListener('load', function() {
            ...
            <?php if ($isEditMode && !empty($editVideoData['id'])): ?>
                ...
                function loadEditModeChannels() {
                    ...
                    console.log('✅ Channel selector ready, fetching channels...');
                    
                    // Fetch channels for this video
                    fetch('./get-video-channels.php?video_id=<?= $editVideoData['id'] ?>')
                        .then(response => response.json())
                        .then(data => {
                            console.log('📺 API Response:', data);
                            
                            if (data.success && data.channels && data.channels.length > 0) {
                                console.log('📺 Loading ' + data.channels.length + ' channels:', data.channels);
                                window.channelSelector.loadChannels(data.channels);
                            } else {
                                console.log('ℹ️ No channels found for this video');
                            }
                        })
                        .catch(error => {

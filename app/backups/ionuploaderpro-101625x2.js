...
class R2MultipartUploader {
    ...
    async uploadPart(part, presignedUrl, retryCount = 0) {
        try {
            ...
            const uploadedSize = this.completedParts.length * this.partSize;
            const progress = (this.completedParts.length / this.parts.length) * 100;
            ...
            this.onProgress({
                loaded: Math.min(uploadedSize, this.file.size),
            ...
            console.log(`✅ Part ${part.partNumber} uploaded successfully (${progress.toFixed(1)}% complete)`);

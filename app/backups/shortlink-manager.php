...
class VideoShortlinkManager {
    ...
    public function __construct($db, $base_url = null) {
        ...
        // Use dynamic base URL instead of hard-coded ions.com
        if ($base_url) {
            $this->base_url = $base_url;
        } else {
            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
            $host = $_SERVER['HTTP_HOST'] ?? 'ions.com';
            $this->base_url = $protocol . '://' . $host . '/v/';
        }
    }
    ...
    public function resolveShortlink($shortlink) {
        try {
            ...
            // Try exact match on shortcode (case-insensitive) - JOIN with IONEERS to get creator info
            $video = $this->db->get_row($this->db->prepare(
                "SELECT v.*, u.handle as creator_handle, u.fullname as creator_name, u.photo_url as creator_photo 
                 FROM IONLocalVideos v 
                 LEFT JOIN IONEERS u ON v.user_id = u.user_id 
                 WHERE LOWER(v.short_link) = %s",
                $shortlink
            ));
            ...
            if (!$video && preg_match('/^[a-z0-9]{6,8}$/', $shortlink)) {
                $video = $this->db->get_row($this->db->prepare(
                    "SELECT v.*, u.handle as creator_handle, u.fullname as creator_name, u.photo_url as creator_photo 
                     FROM IONLocalVideos v 
                     LEFT JOIN IONEERS u ON v.user_id = u.user_id 
                     WHERE LOWER(v.short_link) LIKE %s",
                    $shortlink . '-%'
                ));
            }

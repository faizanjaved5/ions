    ...
    try {
        ...
        if ($is_zip) {
            ...
            $sql = "SELECT slug, city_name, channel_name, population, state_name, state_code, country_name, country_code, latitude, longitude, custom_domain
                    ...
                    WHERE slug IS NOT NULL 
                    AND city_name IS NOT NULL 
                    AND (LOWER(city_name) LIKE ? OR LOWER(state_name) LIKE ? OR LOWER(country_name) LIKE ? OR LOWER(channel_name) LIKE ? OR LOWER(slug) LIKE ? OR LOWER(custom_domain) LIKE ?)
            ...
            error_log("🔍 CHANNEL SEARCH: Text search returned " . (is_array($channels_objects) ? count($channels_objects) : 'NULL') . " results");

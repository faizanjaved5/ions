    ...
    if (step2) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    console.log('📱 Mobile: Attempting to generate auto thumbnail');
                    generateAutoThumbnail();
...
function generateYouTubeThumbnail(videoId) {
    ...
    // Show loading state on thumbnail
    ...
    function tryNextQuality() {
        ...
        fetch(thumbnailUrl, { mode: 'cors' })
            ...
            .then(blob => {
                ...
                // Update preview with visual effect
                const url = URL.createObjectURL(blob);
                
                if (preview) {
                    // Force a visual update even if it's the same image
                    const currentSrc = preview.src;
                    console.log('📷 Current thumbnail src:', currentSrc);
                    console.log('📷 New thumbnail src:', url);
                    
                    // Add flash border effect
                    const previewContainer = preview.parentElement;
                    if (previewContainer) {
                        previewContainer.style.boxShadow = '0 0 0 4px #10b981';
                        previewContainer.style.transition = 'box-shadow 0.3s ease';
                    }
                    
                    // Fade out with stronger visual effect
                    preview.style.transition = 'opacity 0.3s ease, filter 0.3s ease, transform 0.3s ease';
                    preview.style.opacity = '0';
                    preview.style.filter = 'blur(8px)';
                    preview.style.transform = 'scale(0.95)';
                    
                    setTimeout(() => {
                        // Update src - force reload by revoking old URL
                        ...
                        setTimeout(() => {
                            ...
                            console.log('🖼️ YouTube thumbnail preview updated and animated');
                            
                            // Remove flash effect and add "Updated!" badge
                            if (previewContainer) {
                                setTimeout(() => {
                                ...
                                updateBadge.style.cssText = `
                                    ...
                                    z-index: 1000;
                                    animation: updateBadgePop 0.8s ease forwards;
                                `;
                                updateBadge.textContent = '✓ Updated!';
                                
                                // Add animation keyframes
                                if (!document.getElementById('update-badge-animation')) {
                                    const style = document.createElement('style');
                                    style.id = 'update-badge-animation';
                                    style.textContent = `
                                ...
                                previewContainer.appendChild(updateBadge);
                ...
                showSuccessToast('✅ YouTube thumbnail captured successfully!');
            })
            .catch(error => {
...
// Generate Vimeo thumbnail using Vimeo's oEmbed API (CORS-safe)
function generateVimeoThumbnail(videoId) {
    ...
    // Use Vimeo's oEmbed API to get thumbnail URL (CORS-safe)
    ...
    fetch(oembedUrl)
        ...
        .then(data => {
            ...
            // Get the thumbnail URL from oEmbed response
            ...
            // For higher resolution, replace _640 with _1280 if available
            const highResThumbnailUrl = thumbnailUrl.replace('_640', '_1280');
            console.log('📷 Vimeo thumbnail URL (high-res):', highResThumbnailUrl);
            
            // Fetch the thumbnail image
            return fetch(highResThumbnailUrl, { mode: 'cors' })
                .then(response => {
                    if (!response.ok) {
                        // Fallback to original resolution
                        console.log('⚠️ High-res failed, trying standard resolution');
        ...
        .then(blob => {
            ...
            if (preview) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        preview.style.filter = 'none';
                        preview.style.transform = 'scale(1)';
                        console.log('🖼️ Vimeo thumbnail preview updated and animated');
            ...
            showSuccessToast('✅ Vimeo thumbnail captured successfully!');
        })
        .catch(error => {
            ...
            showSuccessToast('❌ Could not load Vimeo thumbnail. The video may be private or unavailable.');
        });
}

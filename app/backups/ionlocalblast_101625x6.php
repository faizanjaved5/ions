...
<body>
    ...
    <script>
        ...
        function displaySelectedChannels() {
            ...
            // Calculate total price based on selected intervals
            const priceValues = { monthly: 3.95, quarterly: 6.95, annual: 24.95 };
            let cartTotal = 0;
            selectedChannels.forEach(channel => {
                const interval = channel.interval || 'monthly';
                cartTotal += priceValues[interval];
            });
                ...
                if (pricing && pricing.pricing) {
                    ...
                    tooltip.innerHTML = `
                        <div style="display: flex; gap: 4px; align-items: center;">
                            <div style="flex: 1; text-align: center; padding: 6px 8px; background: rgba(255,255,255,0.05); border-radius: 4px;">
                                <div style="font-size: 0.7rem; color: #94a3b8; margin-bottom: 2px;">MONTHLY</div>
                                <div style="color: #10b981; font-weight: 700; font-size: 0.9rem;">$${p.monthly}</div>
                            </div>
                            <div style="flex: 1; text-align: center; padding: 6px 8px; background: rgba(255,255,255,0.05); border-radius: 4px;">
                                <div style="font-size: 0.7rem; color: #94a3b8; margin-bottom: 2px;">QUARTERLY</div>
                                <div style="color: #60a5fa; font-weight: 700; font-size: 0.9rem;">$${p.quarterly}</div>
                            </div>
                            <div style="flex: 1; text-align: center; padding: 6px 8px; background: rgba(255,255,255,0.05); border-radius: 4px;">
                                <div style="font-size: 0.7rem; color: #94a3b8; margin-bottom: 2px;">SEMI-ANN.</div>
                                <div style="color: #fbbf24; font-weight: 700; font-size: 0.9rem;">$${p.semi_annual || p.quarterly}</div>
                            </div>
                            <div style="flex: 1; text-align: center; padding: 6px 8px; background: rgba(255,255,255,0.1); border-radius: 4px; border: 1px solid rgba(251, 191, 36, 0.4);">
                                <div style="font-size: 0.7rem; color: #fbbf24; margin-bottom: 2px;">ANNUAL</div>
                                <div style="color: #f59e0b; font-weight: 700; font-size: 0.9rem;">$${p.annual}</div>
                            </div>
                        </div>
                    `;
                } else {

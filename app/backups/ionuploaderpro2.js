...
class R2MultipartUploader {
    constructor(options = {}) {
        // OPTIMIZED 2025-10-17: 100MB parts for 2GB-20GB files
        // - 2GB file = 20 parts
        // - 20GB file = 200 parts  
        // - Faster than 50MB parts for large files
        this.partSize = options.partSize || 100 * 1024 * 1024; // 100MB parts - optimal for 2GB-20GB files
        this.maxConcurrentUploads = options.maxConcurrentUploads || 2; // Upload 2 parts simultaneously for speed
        this.maxRetries = options.maxRetries || 3; // 3 retry attempts
        ...
        this.endpoint = options.endpoint || './ionuploadermultipart3.php';
        ...
        fetch('./check-multipart-version.php')
            ...
            .then(data => {
                if (data.overall_status === 'PASS') {
                    console.log('%c✅ Backend Version Check: ' + data.message, 'color: green; font-weight: bold');
                } else {
                    console.error('%c❌ Backend Version Check: ' + data.message, 'color: red; font-weight: bold');
                    console.error('Details:', data.checks);
                }
            })

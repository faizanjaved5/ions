...
ini_set('error_log', __DIR__ . '/../uploads/r2_multipart_debug.log');

// VERSION CONSTANTS - Used for verification
define('R2_MULTIPART_VERSION', '3.0');
define('R2_MULTIPART_VERSION_DATE', '2025-10-17');
define('R2_MULTIPART_HAS_ENCODING_FIX', true);
...
function handleInitMultipartUpload() {
    ...
    $thumbnailUrl = 'https://ions.com/assets/default/processing.png'; // Default
    if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
        $thumbnailUrl = uploadThumbnail($_FILES['thumbnail']);
        error_log('✅ Thumbnail uploaded: ' . $thumbnailUrl);
    }
...
function handleGetPresignedUrls() {
    ...
    $startPart = intval($_POST['startPart'] ?? 1); // NEW: Support getting URL for specific part
    ...
    // If partCount=1 and startPart=3, generates URL only for part 3
    // If partCount=5 and startPart=1, generates URLs for parts 1-5
    $r2Config = $config['cloudflare_r2_api'];
    $presignedUrls = [];
    
    $endPart = $startPart + $partCount - 1;
...
function handleCompleteMultipartUpload() {
    ...
    try {
        ...
        // Handle multi-channel distribution (same logic as ionuploadvideos.php)
        try {
            $selectedChannels = isset($_POST['selected_channels']) ? json_decode($_POST['selected_channels'], true) : [];
            error_log('📺 MULTIPART: selected_channels POST value: ' . ($_POST['selected_channels'] ?? 'NOT SET'));
            error_log('📺 MULTIPART: decoded selectedChannels: ' . print_r($selectedChannels, true));
            ...
            if (!empty($selectedChannels) && is_array($selectedChannels)) {
                ...
                error_log('ℹ️ No channels selected for distribution');
            }
        } catch (Exception $e) {
        ...
        // Extract shortlink code from URL (e.g., "abc123" from "https://ions.com/v/abc123")
        $shortlink_code = null;
        if ($share_url) {
            if (preg_match('/\/v\/([^\/\?]+)/', $share_url, $matches)) {
                $shortlink_code = $matches[1];
            }
        }
        ...
        $response = [
            ...
            'celebration' => true, // Enable celebration dialog
            'shortlink' => $shortlink_code, // Short link code for celebration dialog
            'share_url' => $share_url, // Full share URL
            'thumbnail' => $session->thumbnail ?: 'https://ions.com/assets/default/processing.png'
        ];
...
function initializeR2MultipartUpload($objectKey, $contentType, $r2Config) {
    ...
    // CRITICAL FIX: Encode object key path the SAME WAY as presigned URLs
    // Encode each path segment but preserve slashes
    $pathSegments = explode('/', $objectKey);
    $encodedSegments = array_map('rawurlencode', $pathSegments);
    $encodedPath = implode('/', $encodedSegments);
    
    $url = "https://$host/$encodedPath?uploads";
    ...
    if ($httpCode !== 200) {
        error_log("❌ R2 multipart init failed - HTTP $httpCode");
        error_log("   Response: $response");
        throw new Exception("Failed to initialize multipart upload: HTTP $httpCode - $response");
    }
    
    error_log("✅ R2 multipart init successful - HTTP $httpCode");
    ...
    error_log("✅ R2 Upload ID obtained: " . substr($uploadId, 0, 30) . "...");
...
function generateR2PresignedUrl($objectKey, $uploadId, $partNumber, $r2Config) {
    ...
    // CRITICAL FIX: Build canonical query string with ALPHABETICAL ORDER
    // AWS requires query params to be sorted alphabetically
    $queryParams = [
    ...
    // CRITICAL: Must use rawurlencode for both keys and values
    $canonicalPairs = [];
    foreach ($queryParams as $key => $value) {
        $canonicalPairs[] = rawurlencode($key) . '=' . rawurlencode($value);
    }
    $canonicalQueryString = implode('&', $canonicalPairs);
    
    // CRITICAL: Encode object key path segments but preserve slashes
    ...
    // CRITICAL FIX: Build presigned URL - use same encoding as canonical URI
    ...
    error_log("   Host: $host");
    error_log("   Object Key: $objectKey");
    error_log("   Canonical URI: $canonicalUri");
    error_log("   Upload ID (length " . strlen($uploadId) . "): " . substr($uploadId, 0, 30) . "..." . substr($uploadId, -30));
    error_log("   Canonical Query (length " . strlen($canonicalQueryString) . ")");
    error_log("   Signature: " . substr($signature, 0, 16) . "...");
    error_log("   FULL URL: " . $presignedUrl);
    error_log("   URL Length: " . strlen($presignedUrl));
...
function uploadThumbnail($file) {
    ...
    return 'https://ions.com/assets/default/processing.png';
}

...
<head>
    ...
    <style>
        ...
        /* MOBILE: Force footer to be visible and accessible */
        @media (max-width: 768px) {
            .modal-footer {
                ...
                background: linear-gradient(135deg, #1b1d23, #282c34) !important;
            ...
            /* Hide Close button on mobile (X in header is enough) */
            ...
            /* Add padding to modal body to prevent content from being hidden under footer */
            .modal-body {
                ...
                <div id="step2" class="step2-content" <?= $isEditMode ? 'style="display: grid;"' : 'style="display: none;"' ?>>
                    ...
                    <div class="video-preview-column">
                        ...
                        <div class="thumbnail-section">
                            <div class="thumbnail-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                <h4 style="margin: 0;">Thumbnail</h4>
                                <div id="thumbnailStatusBadge" style="font-size: 11px; color: #10b981; font-weight: 600; padding: 4px 10px; border-radius: 4px; background: rgba(16, 185, 129, 0.1); display: none; align-items: center; gap: 4px;">
                                    <span>✓</span>
                                    <span id="thumbnailStatusText"></span>
                                </div>
                            </div>
    ...
    <script>
        window.addEventListener('load', function() {
            ...
            <?php if ($isEditMode && !empty($editVideoData['id'])): ?>
                ...
                function loadEditModeBadges() {
                    const badgesString = <?= json_encode($editVideoData['badges']) ?>;
                    console.log('🏷️ Loading badges:', badgesString);
                    
                    if (!badgesString || !badgesString.trim()) {
                        console.log('🏷️ No badges to load');
                        return;
                    }
                    
                    // CRITICAL: Wait for badge UI elements to be ready before populating
                    const badgesContainer = document.getElementById('badgeInput'); // FIXED: Use correct ID
                    const badgesInput = document.getElementById('videoBadges');
                    
                    if (!badgesContainer || !badgesInput) {
                        console.log('⏳ Waiting for badge UI elements to be ready...');
                        console.log('   badgesContainer (badgeInput):', !!badgesContainer);
                        console.log('   badgesInput (videoBadges):', !!badgesInput);
                        setTimeout(loadEditModeBadges, 100); // Retry after 100ms
                        return;
                    }
                    
                    console.log('✅ Badge UI elements ready, populating badges...');
                    
                    const badgeList = badgesString.split(',').map(b => b.trim()).filter(b => b);
                    console.log('🏷️ Parsed badge list:', badgeList);
                    ...
                    badgeList.forEach(badgeName => {
                        ...
                        badgePill.innerHTML = `
                            ${badgeName}
                            <button type="button" class="badge-remove" onclick="removeBadge('${badgeName}')" aria-label="Remove ${badgeName}">×</button>
                        `;
                        badgesContainer.appendChild(badgePill);
                    });
                    
                    // Update hidden input
                    badgesInput.value = badgeList.join(',');
                    
                    console.log('✅ Badges loaded successfully:', badgeList.join(', '));
                }

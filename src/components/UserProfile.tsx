...
const UserProfile = ({ onLogout }: UserProfileProps) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="w-64 bg-[#1a1d21] border-gray-700"
        align="end"
        sideOffset={8}
      >
        ...
        {user.menuItems.map((item, index) => {
          ...
          return (
            <DropdownMenuItem key={index} asChild>
              <a
                href={item.link}
                className="cursor-pointer text-gray-300 hover:text-white hover:bg-gray-800 focus:text-white focus:bg-gray-800 flex items-center px-3 py-2.5"
              >
                {IconComponent && <IconComponent className="mr-3 h-4 w-4" />}
                <span>{item.label}</span>
              </a>

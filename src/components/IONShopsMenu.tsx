...
const getLeagueIconId = (itemName: string): string | null => {
  const league = itemName.toLowerCase().replace(/\s+/g, "-");
...
const schoolLogoMap: Record<string, string> = {
  ...
  "California Polytechnic State University":
    "California_Polytechnic_State_University.png",
...
const IONShopsMenu = () => {
  const [selectedCategory, setSelectedCategory] = useState<string | null>(
    shopsData.categories?.[0]?.id ?? null
  );
  ...
  const bebasStyles = useBebasFont
    ? "font-bebas font-normal whitespace-nowrap uppercase tracking-wider"
    : "";
  ...
  const renderItemsGrid = (items: ShopItem[]) => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-[0.35rem]">
      {items.map((item) => {
        ...
        const localLogo =
          currentCategory?.id === "by-school" ? schoolLogoMap[item.name] : null;

...
try {
    ...
    if ($is_join_request) {
        ...
        if ($existing_user) {
            ...
            error_log("Join request but user exists - redirecting to login");
            $_SESSION['otp_error'] = '❌ An account with this email already exists. Please log in instead.';
            header('Location: /login/index.php');
            exit;
        }
        ...
        if (!$insert_result) {
            ...
            $_SESSION['otp_error'] = '❌ Failed to create account. Please try again.';
        ...
        if (!$existing_user) {
            // User doesn't exist, redirect to join
            error_log("Login attempt but user doesn't exist - redirecting to join");
            $_SESSION['otp_error'] = '❌ No account found. Please sign up first.';
            header('Location: /join/index.php');
            exit;
        }

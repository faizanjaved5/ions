 ...
 * - If user exists with 'Member' role -> redirect to OTP verification
 * - If new user -> create account and proceed to step 2
 * 
 * Google OAuth Integration Notes:
 * The OAuth callback should implement the same logic:
 * 1. Check if user exists by email
 * 2. If exists and role='Guest' -> set $_SESSION['oauth_user_data'] and redirect here with ?oauth_new_user=1
 * 3. If exists and role='Member' -> redirect to OTP verification
...
// Note: The old REQUIRE_OTP_AFTER_PAYMENT constant has been removed
// as OTP is now handled inline based on user role
 ...
 * 
 * When handling the OAuth callback (state='join'):
 * 
 ...
 *            // Not upgraded - send to step 2
 *            $_SESSION['oauth_user_data'] = [
 *                'email' => $google_email,
 *                'fullname' => $google_name,
 *                'user_id' => $existing_user->user_id
 *            ];
 *            ion_login_user($existing_user);
 *            header('Location: /join/?oauth_existing_guest=1');
 *        } else {
 *            // Any other role (Member, Creator, etc.) - need OTP verification
 *            ion_login_user($existing_user);
 *            $_SESSION['pending_otp_email'] = $google_email;
 ...
 *        $new_user = create_user($google_email, $google_name, 'Guest');
 *        $_SESSION['oauth_user_data'] = [...];
 *        ion_login_user($new_user);
 *        header('Location: /join/?oauth_new_user=1');
 *    }
 */
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' && !isset($_GET['oauth_new_user']) && !isset($_GET['oauth_existing_guest'])) {

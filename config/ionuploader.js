...
function showCustomAlert(title, message) {
    // Create a proper modal for HTML content
...
function updateCaptureButtonVisibility() {
    ...
    // Show for uploaded files or in edit mode
    if (window.editVideoData || selectedFile) {
...
function tryAlternativeCapture() {
    ...
    // Try to generate thumbnail from video URL if it's a supported platform
    if (window.editVideoData && window.editVideoData.video_link) {
        ...
        if (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be')) {
            ...
            if (videoId) {
                // For Vimeo, we'll use the existing thumbnail or show the dialog
                console.log('🎬 Vimeo video detected, using existing thumbnail');
                if (window.editVideoData.thumbnail) {
                    updateThumbnailPreview(window.editVideoData.thumbnail);
                    return;
                }
            }
